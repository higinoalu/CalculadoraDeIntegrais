#include <stdio.h>
#include <math.h>

// ----------------------------------------------------------------------------
// Definição do Tipo de Ponteiro de Função
// ----------------------------------------------------------------------------
// Isso nos permite tratar qualquer função que recebe um 'double' e retorna
// um 'double' como um tipo de dado chamado 'Funcao'.
// É a peça central para tornar nosso calculador de integral genérico.
typedef double (*Funcao)(double);

// ----------------------------------------------------------------------------
// Função Principal de Integração (Método dos Retângulos - Ponto Médio)
// ----------------------------------------------------------------------------
/**
 * @brief Calcula a integral definida de uma função f no intervalo [a, b].
 *
 * @param f Ponteiro para a função a ser integrada.
 * @param a Limite inferior de integração.
 * @param b Limite superior de integração.
 * @param n_passos O número de retângulos a serem usados na aproximação.
 * Quanto maior o valor, maior a precisão e o custo computacional.
 * @return O valor aproximado da integral.
 */
double calcular_integral(Funcao f, double a, double b, int n_passos) {
    if (a == b) {
        return 0.0;
    }

    double largura_retangulo = (b - a) / n_passos;
    double soma_areas = 0.0;

    // Itera por cada retângulo, usando o ponto médio para a altura
    for (int i = 0; i < n_passos; i++) {
        // Ponto médio do retângulo atual
        double ponto_medio = a + (i + 0.5) * largura_retangulo;
        // Altura do retângulo é f(ponto_medio)
        double altura = f(ponto_medio);
        // Soma a área do retângulo atual (altura * largura)
        soma_areas += altura * largura_retangulo;
    }

    return soma_areas;
}

// ----------------------------------------------------------------------------
// Exemplos de Funções Elementares para Integração
// ----------------------------------------------------------------------------
// Todas seguem a assinatura definida por 'Funcao': double nome(double x)

// 1. Polinômio: f(x) = x^2
double polinomio_quadratico(double x) {
    return x * x;
}

// 2. Trigonométrica: f(x) = sin(x)
double trigonometrica_seno(double x) {
    return sin(x);
}

// 3. Exponencial: f(x) = e^x
double exponencial(double x) {
    return exp(x);
}

// 4. Logaritmo: f(x) = ln(x)
//    (Domínio: x > 0)
double logaritmo_natural(double x) {
    if (x <= 0) {
        // Retorna um valor inválido (Not-a-Number) se fora do domínio
        return NAN;
    }
    return log(x);
}

// 5. Função Composta (Exponencial + Polinômio): f(x) = e^(-x^2) (Curva de Gauss)
//    Essa função não possui uma integral analítica elementar.
double gaussiana(double x) {
    return exp(-pow(x, 2));
}

// 6. Quociente de funções: f(x) = sin(x) / x
double quociente_sinc(double x) {
    if (x == 0.0) {
        return 1.0; // Limite de sin(x)/x quando x -> 0 é 1
    }
    return sin(x) / x;
}

// ----------------------------------------------------------------------------
// Função Principal (Main) - Ponto de Entrada e Demonstração
// ----------------------------------------------------------------------------
int main() {
    // Definindo a precisão (número de retângulos)
    // Valores altos como 1,000,000 já fornecem excelente precisão para a maioria das funções.
    int n_passos = 1000000;

    printf("Calculadora de Integral Numérica (Método dos Retângulos)\n");
    printf("Usando n = %d passos para a aproximação.\n\n", n_passos);

    // --- Exemplo 1: Polinômio ---
    double a1 = 0.0, b1 = 1.0;
    double res1 = calcular_integral(polinomio_quadratico, a1, b1, n_passos);
    printf("Integral de f(x)=x^2 de %.2f a %.2f = %f\n", a1, b1, res1);
    printf("(Valor exato = 1/3 = 0.333333...)\n\n");

    // --- Exemplo 2: Trigonométrica ---
    double a2 = 0.0, b2 = M_PI; // M_PI é a constante pi da math.h
    double res2 = calcular_integral(trigonometrica_seno, a2, b2, n_passos);
    printf("Integral de f(x)=sin(x) de %.2f a %.2f = %f\n", a2, b2, res2);
    printf("(Valor exato = 2.0)\n\n");

    // --- Exemplo 3: Exponencial ---
    double a3 = 0.0, b3 = 1.0;
    double res3 = calcular_integral(exponencial, a3, b3, n_passos);
    printf("Integral de f(x)=e^x de %.2f a %.2f = %f\n", a3, b3, res3);
    printf("(Valor exato = e - 1 = 1.718281...)\n\n");

    // --- Exemplo 4: Função Composta (Gaussiana) ---
    double a4 = -1.0, b4 = 1.0;
    double res4 = calcular_integral(gaussiana, a4, b4, n_passos);
    printf("Integral de f(x)=e^(-x^2) de %.2f a %.2f = %f\n", a4, b4, res4);
    printf("(Valor aproximado conhecido ~= 1.493648...)\n\n");
    
    // --- Exemplo 5: Logaritmo ---
    double a5 = 1.0, b5 = exp(1.0); // Integrando de 1 até 'e'
    double res5 = calcular_integral(logaritmo_natural, a5, b5, n_passos);
    printf("Integral de f(x)=ln(x) de %.2f a %.2f = %f\n", a5, b5, res5);
    printf("(Valor exato = 1.0)\n\n");

    return 0;
}